...
plt.rcParams['font.sans-serif'] = ['Arial']  # 设置全局字体为 Arial
...
def class_z2(file_path, line_dis, number, dst_svg_path):
    # 读取Excel文件
    df = pd.read_excel(file_path)
    ...
    if closest_distance > line_dis:
        ...
        plt.legend()
        plt.savefig(f'{dst_svg_path}_{number}.svg', format='svg')
...
def class_z_final(file_path, dst_xls_path='step11_dst.xlsx', dst_svg_path='cluster', min_num_clusters=4, max_num_clusters=8):

    Z, df, values = class_z(file_path)
    result = []
    for d in values:
        num_clusters, dst_class, de_count = get_class_by_distance(d, Z, df)
        if min_num_clusters <= num_clusters <= max_num_clusters:
            for c in dst_class:
                dst_class[dst_class.index(c)]['num_clusters'] = num_clusters
                dst_class[dst_class.index(c)]['distance'] = d
                result.append(c)
            class_z2(file_path, d, num_clusters, dst_svg_path)
    save_dict_list_to_excel(result, dst_xls_path)


if __name__ == '__main__':
    src_file_path = 'step8.高频药物矩阵10.xlsx'
